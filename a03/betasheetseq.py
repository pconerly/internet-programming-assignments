...
def parsesheet(s, key, title):
    ...
    while index != -1:
        ...
        temp = region(int(start), int(end), regiontitle)
        regions.append(temp)
        index = s.find(key)
    #print regions
    return regions
...
def parseregion(regions):
    tempseq = []
    if len(regions) == 0:
        return [-1, -1]
    for i in range(len(regions)):
        if regions[i].title == 'Beta-strand region':
            for j in range(regions[i].start-1, regions[i].end):
                tempseq.append(j)
    tempseq.sort()
    h = 0
    regionseq = []
    for h in range(0,tempseq[-1]+1):
        if h in tempseq:
            regionseq.append(5)
        else:
            regionseq.append(-5)
